...
| `iniciar-sesion/index.loader.test.ts` | 6 | Quien ya tiene sesión (el `authPayload` del middleware, no la cookie) va a `/dashboard`, o al escaneo del QR si venía de uno; un `redirectTo` hostil se ignora. |
...
Núcleo de render (F-07), editor (F-08), emisión (F-09), verificación pública (F-10) y entrega al participante (F-11) del certificado. Lo que estas
...
| Archivo | Tests | Qué protege |
|---|---:|---|
| `domain/…/certificate.rules.test.ts` | 59 | Plantilla desconocida, vacía o nula que cae en la institucional; `safeAccent` que solo deja pasar `#rrggbb` (inyección de CSS, hex corto, `url()`); `safeImageUrl` que admite `https:` y el proxy y descarta `javascript:`, `data:`, `blob:` y `http:`; y el esquema del diseño en su tope y con dos firmantes exactos. Desde F-08: `isOwnSignatureRef` solo para la firma del propio curso (rechaza otro curso, `blob:`, `data:`, URL externa, otra carpeta, key sin codificar y escape de carpeta), `canEditCertificate` hasta finalizado, `resolveFolio`, el contraste del logo, y `designsEqual`/`certificateStateOf`. Desde F-09: el folio sin `{seq}` rechazado, `diffIssues` (emitir, idempotente, revocar y restaurar), el esquema de descarga que descarta un diseño en la petición y el nombre de archivo saneado. Desde F-10: la verificación solo acepta un UUID. |
...
| `application/…/certificate-issuance.server.test.ts` | 8 | Emite con el publicado y folios consecutivos, con el de por defecto sin publicado, no duplica al repetir, revoca y restaura con el mismo folio, y sin nadie a quien emitir no reserva folios. Desde F-11: un correo por emisión nueva con el mensaje del curso, ninguno al restaurar o revocar, y nada encolado si la emisión falla. |
...
| `domain/templates/__tests__/qr.test.ts` | 2 | El QR es un SVG plano (un fondo y un trazo, sin imágenes ni scripts) y determinista. |
| `routes/verificar/$documentId/__tests__/` | 7 | Responde sin sesión; **la respuesta no trae correo ni ids** (probado sobre la forma); revocado distinto de inexistente; malformado da 404 sin llegar al servicio; 429 pasado el límite y por IP; y las cabeceras `noindex` y `no-store`. |
| `routes/mis-certificados/**/__tests__/` | 7 | La lista pide solo los de la sesión aunque la URL diga otra cosa, un externo sin dependencia entra y sin sesión redirige; la descarga propia ignora un diseño en la query y responde 403 apagada y 404 ajena. |
